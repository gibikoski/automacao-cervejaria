# T1_1_Integrar_Leitura_Temperatura_DS18B20  
...
- **Código Associado**: [T1_1_Integrar_Leitura_Temperatura_DS18B20.ino](./T1_1_Integrar_Leitura_Temperatura_DS18B20.ino)  
- **Diretório**: docs/V1.01/codes/Controle_de_Fermentacao/T1_1  

## Descrição  
Este teste integra a leitura de temperatura com o sensor DS18B20, exibindo valores no Serial Monitor com validação de faixa (0–50 °C).  

## Setup  
- Conectar o Arduino R4 Connect via USB ou fonte 7-12V.  
- Conectar o DS18B20 ao pino 10:  
  - Dados (DQ) ao pino 10, resistor pull-up 4.7 kΩ entre DQ e VCC (3.3V/5V).  
  - VCC ao 3.3V/5V, GND ao GND.  
- Instalar bibliotecas `OneWire` e `DallasTemperature` no Arduino IDE.  
- Carregar o sketch e abrir o Serial Monitor (9600 baud).  

## Resultados Esperados  
- Serial Monitor exibe:  
  - "Teste T1.1 - Integração Leitura Temperatura DS18B20 Iniciado".  
  - "Sensor DS18B20 detectado."  
  - A cada 2 segundos:  
    - Temperatura (ex.: "Temperatura: 25.50 °C (Faixa válida)").  
    - Se fora de 0–50 °C: "Temperatura: 55.00 °C (Fora da faixa esperada)".  
  - Se desconectado: "Erro: Sensor DS18B20 desconectado!".  

## Notas  
- Se o sensor não for detectado, verificar conexões e resistor pull-up.  
- Valida o requisito RD5.1 (integração de leitura de temperatura).  
