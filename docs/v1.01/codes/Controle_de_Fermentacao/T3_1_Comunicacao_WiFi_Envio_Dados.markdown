# T3_1_Comunicacao_WiFi_Envio_Dados  
...
- **Código Associado**: [T3_1_Comunicacao_WiFi_Envio_Dados.ino](./T3_1_Comunicacao_WiFi_Envio_Dados.ino)  
- **Diretório**: docs/V1.01/codes/Controle_de_Fermentacao/T3_1  

## Descrição  
Este teste verifica a comunicação WiFi do Arduino R4 Connect, simulando o envio de dados (ex.: temperatura) e exibindo o status no Serial Monitor.  

## Setup  
- Conectar o Arduino R4 Connect via USB ou fonte 7-12V.  
- Editar o código para incluir o SSID e senha da rede WiFi (`SUA_REDE_WIFI` e `SUA_SENHA_WIFI`).  
- Carregar o sketch e abrir o Serial Monitor (9600 baud).  
- Garantir que o Arduino esteja no alcance de uma rede WiFi funcional.  

## Resultados Esperados  
- Serial Monitor exibe:  
  - "Teste T3.1 - Comunicação WiFi Envio Dados Iniciado".  
  - Pontos (".") durante a conexão.  
  - Após conectar:  
    - "Conexão WiFi estabelecida!"  
    - "Endereço IP: [ex.: 192.168.1.100]".  
  - A cada 5 segundos:  
    - "Enviando dado simulado (Temperatura: 25.5 °C)".  
    - "Dado enviado com sucesso (simulado)".  

## Notas  
- Caso a conexão WiFi falhe, verificar credenciais, sinal e roteador.  
- Este teste simula o envio de dados; um servidor real pode ser integrado posteriormente.  
- Valida o requisito RD7.1 (comunicação WiFi).  
