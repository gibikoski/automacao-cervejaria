# T0_4_Verificar_Leitura_Corrente_INA219  
...
- **Código Associado**: [T0_4_Verificar_Leitura_Corrente_INA219.ino](./T0_4_Verificar_Leitura_Corrente_INA219.ino)  
- **Diretório**: docs/V1.01/codes/Controle_de_Fermentacao/T0_4  

## Descrição  
Este teste verifica a leitura de corrente usando o sensor INA219 conectado ao Arduino R4 Connect via I2C, exibindo corrente (mA) e tensão (V) no Serial Monitor.  

## Setup  
- Conectar o Arduino R4 Connect via USB ou fonte 7-12V.  
- Conectar o sensor INA219:  
  - SDA ao pino A4, SCL ao pino A5 do Arduino.  
  - VCC ao 3.3V ou 5V, GND ao GND.  
  - Conectar uma carga de teste (ex.: resistor ou motor pequeno) em série com o INA219 (terminais Vin+ e Vin-).  
- Instalar a biblioteca `Adafruit_INA219` no Arduino IDE.  
- Carregar o sketch no Arduino IDE e abrir o Serial Monitor (9600 baud).  

## Resultados Esperados  
- O Serial Monitor exibe:  
  - "Teste T0.4 - Leitura Corrente INA219 Iniciado".  
  - "Sensor INA219 inicializado com sucesso."  
  - A cada 2 segundos:  
    - Corrente (ex.: "Corrente: 150.25 mA").  
    - Tensão (ex.: "Tensão: 5.10 V").  
- Se o sensor não inicializar, exibe: "Erro: Falha ao inicializar o INA219!".  

## Notas  
- Caso o sensor não inicialize ou as leituras sejam inconsistentes, verificar conexões I2C, alimentação e a carga conectada.  
- Este teste valida o requisito RD4.3 (leitura de corrente confiável).  
