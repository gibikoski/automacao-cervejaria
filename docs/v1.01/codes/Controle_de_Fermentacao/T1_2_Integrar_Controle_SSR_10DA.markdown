# T1_2_Integrar_Controle_SSR_10DA  
...
- **Código Associado**: [T1_2_Integrar_Controle_SSR_10DA.ino](./T1_2_Integrar_Controle_SSR_10DA.ino)  
- **Diretório**: docs/V1.01/codes/Controle_de_Fermentacao/T1_2  

## Descrição  
Este teste integra o controle do relé SSR-10DA com base em uma condição simulada (ex.: temperatura > 25°C), exibindo o estado do relé no Serial Monitor.  

## Setup  
- Conectar o Arduino R4 Connect via USB ou fonte 7-12V.  
- Conectar o relé SSR-10DA:  
  - Terminal de controle (+) ao pino 9 do Arduino.  
  - Terminal de controle (-) ao GND do Arduino.  
  - Lado de carga (opcional): conectar uma carga de teste (ex.: lâmpada de baixa potência) com alimentação externa (ex.: 220V AC, com cuidado).  
- Carregar o sketch no Arduino IDE e abrir o Serial Monitor (9600 baud).  
- **Atenção**: Manipular circuitos de alta tensão com cuidado, garantindo isolamento e segurança.  

## Resultados Esperados  
- O Serial Monitor exibe:  
  - "Teste T1.2 - Integração Controle SSR-10DA Iniciado".  
  - A cada 2 segundos:  
    - "Temperatura simulada: 26.5 °C".  
    - "Relé SSR-10DA: LIGADO (Temperatura > 25°C)".  
- Se a temperatura simulada for alterada para <= 25°C (editando o código), exibe:  
  - "Relé SSR-10DA: DESLIGADO (Temperatura <= 25°C)".  
- Se uma carga estiver conectada, ela deve ligar/desligar conforme o estado do relé.  

## Notas  
- Para testar com outras temperaturas, editar o valor de `simulatedTemp` no código.  
- Caso o relé não alterne, verificar conexões e alimentação do SSR-10DA.  
- Este teste valida o requisito RD5.2 (integração de controle de saída).  
