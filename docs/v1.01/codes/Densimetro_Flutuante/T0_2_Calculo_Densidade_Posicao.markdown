# T0_2_Calculo_Densidade_Posicao  
...
- **Código Associado**: [T0_2_Calculo_Densidade_Posicao.ino](./T0_2_Calculo_Densidade_Posicao.ino)  
- **Diretório**: docs/V1.01/codes/Densimetro_Flutuante/T0_2  

## Descrição  
Este teste calcula a densidade de um líquido com base na posição do densímetro flutuante, medida pelo sensor VL53L0X, usando a equação da flutuação (massa/volume deslocado). Os resultados são exibidos no Serial Monitor.  
...
- Posicionar o densímetro flutuante em um líquido (ex.: água, densidade ~1000 kg/m³).  
- Definir no código a massa (`DENSIMETER_MASS`) e área transversal (`DENSIMETER_AREA`) do densímetro.  

## Resultados Esperados  
- Serial Monitor exibe:  
  - "Teste T0.2 - Cálculo Densidade Posição Iniciado".  
  - "Posicione o densímetro na posição de referência e pressione qualquer tecla...".  
  - Após tecla: "Calibração concluída. Offset: [valor] mm".  
  - A cada 1 segundo: "Deslocamento: [valor] mm | Densidade: [valor] kg/m³".  
  - Exemplo: Para água, densidade ~1000 kg/m³.  
  - Se falhar: "Erro: Timeout na leitura!".  

## Notas  
- Ajustar `DENSIMETER_MASS` e `DENSIMETER_AREA` conforme o densímetro real.  
- Verificar estabilidade do densímetro no líquido para leituras precisas.  
- Valida o requisito RD2.1 (cálculo de densidade).  
...
- **Balança de Volume**  
  ...
  - T0.2: c0afaf04-d934-4c2c-aeac-60a0a WHICH5b762004  
