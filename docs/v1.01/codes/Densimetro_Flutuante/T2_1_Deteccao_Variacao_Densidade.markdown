# T2_1_Deteccao_Variacao_Densidade  
...
- **Código Associado**: [T2_1_Deteccao_Variacao_Densidade.ino](./T2_1_Deteccao_Variacao_Densidade.ino)  
- **Diretório**: docs/V1.01/codes/Densimetro_Flutuante/T2_1  

## Descrição  
Este teste detecta variações na densidade do líquido, alertando no LCD e Serial Monitor se a variação exceder 5% em relação à leitura anterior.  
...
- Posicionar o densímetro flutuante em um líquido; alterar densidade (ex.: adicionar sal) para testar.  

## Resultados Esperados  
- LCD exibe:  
  - Linha 1: "Densimetro".  
  - Linha 2: "Dens: [valor] kg/m3" (ex.: Dens: 1000 kg/m3), atualizado a cada 0,5 segundo.  
  - Se variação > 5%: "Variação Alta!" por 2 segundos.  
  - Se falhar: "Erro Sensor!" ou "Erro Leitura!".  
- Serial Monitor exibe:  
  - "Teste T2.1 - Detecção Variação Densidade Iniciado".  
  ...
  - A cada 0,5 segundo: "Densidade: [valor] kg/m³".  
  - Se variação > 5%: "Alerta: Variação de densidade > 5%!".  
  - Ao pressionar o botão: "Recalibração concluída. Novo offset: [valor] mm".  
  - Se falhar: "Erro: Timeout na leitura!".  

## Notas  
- Testar com variações controladas (ex.: água para água salgada).  
- Ajustar `VARIATION_THRESHOLD` se necessário.  
- Valida o requisito RD4.1 (detecção de variação).  
