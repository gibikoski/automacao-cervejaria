# T3_1_Sistema_Completo_Medicao_Volume  
...
- **Código Associado**: [T3_1_Sistema_Completo_Medicao_Volume.ino](./T3_1_Sistema_Completo_Medicao_Volume.ino)  
- **Diretório**: docs/V1.01/codes/Balanca_de_Volume/T3_1  

## Descrição  
Este teste integra o HX711, display LCD 16x2, botão de tara e comunicação WiFi, exibindo peso/volume no LCD, Serial Monitor e simulando envio via WiFi.  
...
- Editar o código para incluir SSID e senha da rede WiFi.  
- Instalar bibliotecas `HX711`, `LiquidCrystal`.  
...
- LCD exibe:  
  ...
  - Ao pressionar botão: "Tara realizada" (1 segundo), depois volta às medições.  
- Serial Monitor exibe:  
  - "Teste T3.1 - Sistema Completo Medição Volume Iniciado".  
  - Pontos (".") durante conexão WiFi.  
  - "WiFi conectado. IP: [ex.: 192.168.1.100]".  
  - A cada 2 segundos:  
    - "Peso: [valor] g | Volume: [valor/1000] L".  
    - "Dados enviados (simulado): Peso=[valor]g, Volume=[valor/1000]L".  
  - Ao pressionar botão: "Tara realizada.".  

## Notas  
- Verificar conexões, WiFi, e alimentação se houver falhas.  
- Ajustar contraste do LCD para visibilidade.  
- Valida o requisito RD5.1 (sistema completo).  
