...
- **Código Associado**: [T0_3_Validar_Faixa_Medicao.ino](./T0_3_Validar_Faixa_Medicao.ino)  
- **Diretório**: docs/V1.01/codes/Balanca_de_Volume/T0_3  

## Descrição  
Este teste valida se o peso medido pelo HX711 está dentro da faixa esperada (0–5000 g), exibindo o resultado no Serial Monitor.  
...
- Serial Monitor exibe:  
  - "Teste T0.3 - Validação Faixa Medição Iniciado".  
  - A cada 1 segundo:  
    - "Peso: [valor] g (Faixa válida)" (se 0 ≤ peso ≤ 5000 g).  
    - "Peso: [valor] g (Fora da faixa esperada)" (se fora da faixa).  

## Notas  
- Ajustar `MAX_WEIGHT` conforme a célula de carga usada.  
- Se leituras forem imprecisas, verificar calibração e estabilidade.  
- Valida o requisito RD2.2 (validação de faixa).  
