...
- **Código Associado**: [T0_3_Comunicacao_WiFi.ino](./T0_3_Comunicacao_WiFi.ino)  
- **Diretório**: docs/V1.01/codes/Controle_e_Monitoramento/T0_3  

## Descrição  
Este teste estabelece conexão WiFi e simula o envio de dados de um sensor analógico (escala 0-100) para monitoramento remoto, exibindo valores no LCD e Serial Monitor.  
...
- Instalar biblioteca `WiFi` e `LiquidCrystal`.  
- Carregar o sketch e abrir o Serial Monitor (9600 baud).  

## Resultados Esperados  
- LCD exibe:  
  - Linha 1: "Conectando WiFi".  
  - Linha 2: Pontos (".") durante conexão, depois "IP:[endereço]" ou "Erro WiFi!".  
  - Após conexão: Linha 2 mostra "Valor: [valor]" (ex.: Valor: 50.0).  
- Serial Monitor exibe:  
  - "Teste T0.3 - Comunicação WiFi Iniciado".  
  - "Conectando à rede: [SSID]".  
  - Pontos (".") durante conexão.  
  - Se sucesso: "WiFi conectado. IP: [ex.: 192.168.1.100]".  
  - A cada 2 segundos: "Sensor: [valor] (enviado remotamente)".  
  - Se falhar: "Erro: Falha ao conectar ao WiFi!".  

## Notas  
- **Alerta**: O pino A0 simula um sensor genérico, pois o BOM não especifica sensores. Validar com o BOM para substituir por hardware real.  
- Ajustar credenciais WiFi conforme necessário.  
- Simula envio remoto via Serial; um servidor real pode ser implementado posteriormente.  
- Valida o requisito RC2.2 (comunicação remota).  
