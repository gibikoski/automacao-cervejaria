...
- **Código Associado**: [T1_2_Registro_Eventos.ino](./T1_2_Registro_Eventos.ino)  
- **Diretório**: docs/V1.01/codes/Controle_e_Monitoramento/T1_2  

## Descrição  
Este teste registra eventos (relé ligado/desligado) na EEPROM com base em um sensor analógico (escala 0-100), com limite fixo de 50, exibindo no Serial Monitor.  
...
- Instalar biblioteca `EEPROM`.  
- Carregar o sketch e abrir o Serial Monitor (9600 baud).  

## Resultados Esperados  
- Serial Monitor exibe:  
  - "Teste T1.2 - Registro Eventos Iniciado".  
  - A cada 1 segundo: "Sensor: [valor] | Relé: [LIGADO/DESLIGADO]".  
  - Ao mudar estado do relé: "Evento registrado: Relé [LIGADO/DESLIGADO], Valor: [valor], Evento #[número]".  
  - Após 50 eventos: Não registra mais eventos.  

## Notas  
- **Alerta**: O pino A0 e relé são placeholders, pois o BOM não especifica sensores ou atuadores. Validar com o BOM.  
- Limite de 50 eventos para evitar overflow na EEPROM.  
- Para ler eventos, um código adicional pode ser implementado.  
- Valida o requisito RC3.2 (registro de eventos).  
